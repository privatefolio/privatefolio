...
import {
  ...
  getBlockExplorerUrl,
  getPlatformIdFromCoingeckoId,
} from "src/settings"
import { formatDate, formatDateRelative } from "src/utils/formatting-utils"
          ...
          <Stack gap={2}>
            ...
            {metadata.genesis_date && (
              <div>
                ...
                <span>{formatDate(new Date(metadata.genesis_date))}</span>
                <Typography variant="inherit" color="text.secondary" component="span">
                  {" "}
                  ({formatDateRelative(new Date(metadata.genesis_date))})
                </Typography>
              </div>
            )}
            {/* last_updated */}
      ...
      {metadata.detail_platforms && Object.entries(metadata.detail_platforms).length > 0 && (
        <Paper sx={{ alignSelf: "flex-start", paddingY: 0.5 }}>
          <Table sx={{ width: "fit-content" }} size="small">
            ...
            <TableBody>
              {Object.entries(metadata.detail_platforms).map(([geckoAssetPlatform, details]) => {
                if (!details.contract_address) return null

                const platformId = getPlatformIdFromCoingeckoId(geckoAssetPlatform)

                return (
                  <TableRow key={geckoAssetPlatform}>
                    <TableCell>
                      <IdentifierBlock
                        label={
                          <PlatformBlock platform={platformId || geckoAssetPlatform} size="small" />
                        }
                        id={geckoAssetPlatform}
                        href={`https://www.coingecko.com/en/all-cryptocurrencies?filter_asset_platform=${geckoAssetPlatform}`}
                        linkText="See all assets on this platform"
                        size="small"
                      />
                    </TableCell>
                    <TableCell>
