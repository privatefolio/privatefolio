...
export function generateCandleData(numberOfPoints = 250): CandleData[] {
  const lineData = generateLineData(numberOfPoints)
  return lineData.map((d, i) => {
    const randomRanges = [-1 * Math.random(), Math.random(), Math.random()].map((j) => j * 10)
...
export function generateAlternativeCandleData(numberOfPoints = 250): CandleData[] {
  const lineData = generateLineData(numberOfPoints)
...
export function shuffleValuesWithLimit<T extends WhitespaceData[]>(arr: T, limit: number): T {
  const n = arr.length
  const originalTimes = arr.map((item) => item.time)
  for (let i = 0; i < n; i++) {
    // Generate a random index within the limit
    const j = Math.floor(Math.random() * (Math.min(n - 1, i + limit) - i + 1)) + i
