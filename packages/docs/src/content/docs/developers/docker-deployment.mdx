...
Are you looking to self-host Privatefolio using the official Docker image? Go to [Self-hosting](/users/self-hosting-with-docker).
...
import dockerFile from '../backend/Dockerfile?raw';

<Code code={dockerFile} lang="docker" title="packages/backend/Dockerfile" />
...
Learn more on [self-hosting with Docker](/users/self-hosting-with-docker).

## Workflow file

import workflowFile from '../../.github/workflows/publish-docker-image.yml?raw';

<Code code={workflowFile} lang="yaml" title=".github/workflows/publish-docker-image.yml" mark={[
  { range: "32-37" },
  { range: "39-51" },
  { range: "53-54" },
  { range: "56-69" },
